...
static FLATTEN bool box_get_aabb(rt_float time_start, rt_float time_end, AABB *output, const Box *self)
{
    (void)time_start;
    (void)time_end;
...
static bool box_destroy(Box *self)
{
    size_t i;
    for (i = 0; i < self->list.child_count; i++)
    {
        if (self->list.childs[i].destroy)
        {
            self->list.childs[i].destroy(self->list.childs[i].data);
        }
...
static FLATTEN bool hit_box_object_callback(Ray ray, rt_float t_min, rt_float t_max, HitRecord *record, const Box *self)
{
...
Object box_create(Vec3 posa, Vec3 posb, Material box_material)
{
    ...
    res.destroy = (ObjectDestroy)box_destroy;
    res.collide = (ObjectCallback)hit_box_object_callback;
    res.is_leaf = true;
