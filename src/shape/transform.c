...
Object transform(Object translated, Matrix4x4 matrix)
{
    ...
    b->transformed_aabb.min = vec3_sub(b->transformed_aabb.min, vec3_create(100, 100, 100));
    b->transformed_aabb.max = vec3_sub(b->transformed_aabb.max, vec3_create(-100, -100, -100));
    return res;
}
